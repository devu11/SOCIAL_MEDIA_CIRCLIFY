 ...
 const [postsPerPage] = useState(5);
    ...
    return (
      ...
      <div className="paginationUsers">
        <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1}>Prev <FaLessThan /></button>
        <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === Math.ceil(filteredPosts.length / postsPerPage)}><FaGreaterThan /> Next</button>
      </div>
    </div>
 );
}
